...
3.  **自律的品質ゲート (Hook System)**:
    - **常時監視と検証**: ファイル変更やタスク完了をトリガーに、lint, typecheck, test, review を自動実行。
    - **リスク回避**: 破壊的変更や認証・DB関連の重要な変更を自動検出し、エージェントに適切なガイダンスを提供または進行を一時ブロックします。
...
### 1. 多重起動抑制とライフサイクル・レジリエンス (Process Integrity)
ローカル LLM や Embedding モデルはリソース消費が激しく、複数プロセスからの同時呼び出しはシステムの不安定化を招きます。Gnosis は次の仕組みで実行の整合性を担保します。
- **グローバルセマフォ**: OS レベルのファイルシステム・ロックを利用し、システム全体の同時実行数を厳格に制御。
- **PIDロック & 自動クリーンアップ**: 二重起動を確実に阻止し、異常終了時も PID ファイルとロックを自動修復。
- **Graceful Shutdown**: SIGINT/SIGTERM を捕捉し、進行中の DB 接続やバックグラウンドワーカーを安全にクローズ。
- **Watchdog**: 終了処理のスタックを防ぐ 10 秒の強制終了タイマー。

### 2. イベント駆動型オートメーション (Gnosis Hook)
受動的な記憶検索を超え、作業の「節目」で能動的に介入します。
- `task_checkpoint` を通じた進捗同期と品質検証の強制。
- 変更内容に基づいたリスク・ガイダンスの動的注入。
- 成功/失敗体験の `candidate queue` への自動積載。
- 重複実行を DB レベルで防ぐ**冪等性担保 (hook_executions)**。

### 3. 自律的な情報整理サイクル (Autonomous Promotion Cycle)
...
| **Gnosis Hook** | イベントベースの自動検証・リスクガード・昇格フロー (YAML定義) |
...
| **Async Toolset** | 重い処理をバックグラウンド化し、エージェントの思考を止めない非同期 MCP ツール群 |
...
bun run hooks:setup      # Hook/ルール設定をエージェント環境へ配布
...
- [Gnosis Hook ガイド](docs/hooks-guide.md)
