...
describe('local LLM CLI launchers', () => {
  test('package scripts expose the three command wrappers', () => {
  ...
  test('PATH helpers include all command names', () => {
    ...
    expect(readText('services/local-llm/scripts/install_path.sh')).toContain(
      "To use 'gemma4', 'ollama-v4', 'bonsai', and 'bedrock' commands from anywhere",
    );
  });

  test('setup guidance references all three launchers', () => {
