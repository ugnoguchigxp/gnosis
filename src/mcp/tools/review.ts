...
const reviewParamsSchema = z.object({
  ...
  knowledgePolicy: KnowledgePolicySchema.optional().describe('knowledge retrieval policy'),
...
async function runReviewViaCli(
  ...
  if (parsed.knowledgePolicy) {
    args.push('--knowledge-policy', parsed.knowledgePolicy);
  }
  if (goalText) {
      ...
      const runReview = async () => {
        try {
          ...
          const request = ReviewRequestSchema.parse({
            ...
            mode: parsed.mode,
            taskGoal: goalText,
            knowledgePolicy: parsed.knowledgePolicy ?? 'best_effort',
          });
