        ...
        <table>
            ...
            <tbody>
                {#if loading && tasks.length === 0}
                    <tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 2rem;">Loading tasks...</td></tr>
                {:else if displayTasks.length === 0}
                    <tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 2rem;">No tasks in this category</td></tr>
                {:else}
                    {#each displayTasks as task (task.id)}
                        <tr class="clickable-row" onclick={() => void openDetail(task)}>
