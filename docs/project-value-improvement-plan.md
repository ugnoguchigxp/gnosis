...
- MCP primary surface は `initial_instructions / agentic_search / search_knowledge / record_task_note / review_task / doctor / memory_search / memory_fetch` の8件に整理されている。`memory_search` / `memory_fetch` は context 圧縮回避の補助導線であり、通常の知識取得主導線ではない。
