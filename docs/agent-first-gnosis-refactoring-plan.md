...
9. Episode は primary memory から除外し、episode 統合 LLM コストを中核から外す。
...
- Episode 記憶を active memory として扱わない。
...
### 3.4 Episode の費用対効果が低い

Episode は raw events を LLM でナラティブ化するため、継続運用するとコストが大きい。LLM にとって有用なのはナラティブではなく、抽出済みの rule / lesson / procedure / skill / decision / risk である。

Episode は active memory ではなく、廃止または一時的な provenance に限定する。

### 3.5 Hooks 由来データが低情報量になりやすい

hook candidates は metadata 羅列または episode 化に寄りやすい。
...
| Deprecated | `consolidate_episodes`, episode synthesis tools | 原則廃止。既存データ移行・監査用途のみ |
...
Episode は primary memory として扱わない。

方針:

1. `episode` は `KnowledgeKind` から外す。
2. `consolidate_episodes` は primary/advanced workflow から外し、deprecated とする。
3. hook promotion は episode を生成せず、`lesson`, `decision`, `rule`, `procedure`, `skill`, `risk`, `command_recipe` 候補を直接生成する。
4. 既存 episode データは即削除せず、移行期間中に lesson/decision/procedure 候補へ抽出する。
5. 新規 episode 統合 LLM 呼び出しはデフォルト無効にする。
6. provenance が必要な場合は、episode ではなく raw event references / evidence links として保持する。

削除候補:

- `consolidate_episodes`
- `memoryType: 'episode'`
- episode proxy entity
- `learned_from -> episode` relation
- episode consolidation cron / strict batch
...
自動取得機能のターゲットは、一般的な事実収集や episode 生成ではなく、LLM が再利用できる knowledge candidate の抽出に絞る。
...
- episode narrative だけで本文が構成されていない。
...
| episode consolidation LLM calls | default 0 |
...
11. hooks 由来 candidate が metadata 羅列や episode narrative のまま active knowledge にならないか。
...
20. episode 廃止移行中に hooks / monitor / record_outcome の回帰がないか。
...
- episode 依存コンポーネント移行順（hooks / monitor / CLI / record_outcome）。
...
4. episode 依存系（hooks / monitor / record_outcome）は feature flag 下で A/B テストする。
...
- [x] episode 依存コンポーネント inventory が作成済みで、移行順序が確定済み。
...
### Phase F: Episode Deprecation and Candidate Extraction

- episode consolidation をデフォルト無効化する。
- `consolidate_episodes` を deprecated にする。
- 既存 episode データから lesson/decision/procedure 候補を抽出する migration を用意する。
...
- hooks / monitor / CLI / record_outcome の episode 依存処理を段階移行し、feature flag で切替える。
...
| episode を廃止しすぎて provenance が失われる | なぜその lesson が作られたか追えない | raw event references / evidence links を残す |
...
12. Episode 実装を deprecated / 削除候補にする判断は妥当か。
