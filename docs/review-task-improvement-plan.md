...
- `provider` 未指定の `review_task` が OpenAI に落ち、`OPENAI_API_KEY` 不在で失敗した。
...
1. `review_task` の既定は local-first にする。
2. Cloud provider は明示指定または明示許可がある場合だけ使う。
3. Local LLM が遅い/壊れている場合でも、MCP call を timeout させず structured degraded result を返す。
...
現状のテストには `review_task uses OpenAI as the default MCP reviewer` があり、MCP の既定が OpenAI であることを前提にしている。これは local-first 方針と衝突する。

改善:

- `review_task` の provider 未指定時は `local` として扱う。
- `GNOSIS_REVIEWER` が設定されている場合はそれを優先する。ただし cloud alias の場合は `GNOSIS_REVIEW_ALLOW_CLOUD=true` または explicit `provider` が必要。
- `provider: openai|bedrock|azure-openai` は明示指定時のみ許可する。
- local 失敗時に OpenAI へ暗黙 fallback しない。fallback するなら設定名とログに明示する。

受け入れ基準:

- `provider` 未指定の `review_task` が `getReviewLLMService('local', ...)` を呼ぶ。
- OpenAI API key がなくても、provider 未指定の smoke test が cloud 設定エラーで落ちない。
- explicit `provider: openai` は既存どおり cloud を呼べる。
...
- local LLM failure でも `review.completed` hook は「degraded completed」として発火するか、発火しないなら理由を明示する。
...
- hooks の挙動が docs とテストで一致する。

## 実装順

### Phase 1: 同期 MCP path を壊れない状態にする

1. `review_task` の provider default を local に変更する。
...
5. 既存テスト `review_task uses OpenAI as the default MCP reviewer` を local default テストへ置き換える。

完了条件:

- `review_task` provider 未指定 smoke が OpenAI API key 不在で失敗しない。
...
sync MCP tool で 120 秒以上待つ設計は避ける。Gemma4 が遅いレビューに有用なら、別途 queued review を設計する。

候補:

- `review_task` に `mode: "sync" | "queued"` を追加する。
- sync は 90 秒以内に結果または degraded を返す。
...
4. `review_task` provider 未指定で OpenAI API key error にならない。
...
- `review_task` の既定 provider は local。
- Cloud review は explicit provider または明示 env opt-in。
...
- provider 未指定で OpenAI API key を要求しない。
