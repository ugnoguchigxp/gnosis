...
`agentic_search` が `userRequest`, `files`, `changeTypes`, `technologies` から、今回使う可能性のある Golden Path / failure pattern を必要時だけ候補として返す。

保存はしない。ここでは候補だけを持つ。

#### 実装中の知識候補化

実装中に再利用可能な知見が明確になった場合だけ、verify 後に `record_task_note` の候補として以下を整理する。
...
関連 verify gate が通り、ユーザーが commit を承認したタイミングで、Failure Firewall の候補生成 service が以下を最大 3 件まで提案する。
...
agent は候補を毎回 0-3 件だけ提案する。多すぎる候補は品質が落ちるため、1 タスクあたり active 昇格は原則 1 件までにする。
...
verify 後の候補生成 service が、採用済み review finding や手戻り情報から pattern candidate を作る。
...
- `review_task` の `goal` に `failure_firewall` を含めると FastGate として実行する
- 通常 review では reviewer 内部 tool `lookup_failure_firewall_context` を必要時だけ参照する
- Agentic Search は該当しそうな場合だけ Failure Firewall context を補助情報として返す
- CLI alias `bun run failure-firewall` は既定で MCP host private request 経由で実行する
...
- shared MCP host 経由の private request で実行でき、primary tool は増えない
...
- verify 合格後の候補生成 service から `record_task_note` 用 payload を返す
...
- verify 合格とユーザー commit 承認後に success candidate / failure candidate を 0-3 件提案する
- `review_task` 結果から success / failure / false positive feedback を自動候補化する
- `record_task_note` は `failure-firewall` / `golden-path` tags の候補を `needs_review` として保存する
...
- verify 後候補生成 service から pattern を作る
...
10. verify 後候補生成 service から Golden Path 候補化する。
11. CLI に接続する。
...
ユーザーに毎回手動登録を求めると続かない。`review_task` と verify 後候補生成 service から success / failure candidate を自動生成し、active 昇格だけを人間または高信頼ルールに委ねる。
