...
Gnosis Hook は、Agent-First ツールの開始・完了・レビューイベントや「ファイル変更」をトリガーに、自動的に品質検証、リスク検知、記憶候補の作成を行う自律型オートメーション・システムです。
...
### 主要イベント
- **`task.started`**: `start_task` でタスクトレースを開始したタイミング。
- **`task.completed`**: `finish_task` で成果、検証、学習項目を確定したタイミング。
- **`review.completed`**: `review_task` によるコード・ドキュメントレビューが完了したタイミング。
- **`file.changed`**: ファイル変更を Hook dispatcher へ明示的に渡したタイミング（自動デバウンス機能付き）。
...
エージェントは、Agent-First の公開ツールを通常通り使うだけで Hook イベントを発火できます。

1. **作業開始時**: `start_task` で `task.started` を発火し、対象ファイルや作業モードを Hook context に渡します。
2. **レビュー時**: `review_task` 完了後に `review.completed` を発火し、検出件数や対象ファイルを Hook context に渡します。
3. **タスク終了時**: `finish_task` で `task.completed` を発火し、成果、検証、学習件数を Hook context に渡します。

`file.changed` は現時点では常駐ファイルウォッチャーではなく、Hook dispatcher に変更イベントを渡した場合に処理されます。

---

## 5. Candidate Queue（知識候補の昇格待ち）

Hook によって作成された教訓候補は、即座にデータベースへ保存されるのではなく、まず **Candidate（候補）** として `hook_candidates` テーブルに積まれます。
