...
dry-run は既定で deterministic な順位だけを使うため、ローカル LLM を起動せずに短時間で候補確認できます。LLM 再順位付け込みで dry-run したい場合だけ `--use-llm` を付けます。

```bash
bun src/services/knowflow/cli.ts seed-frontier --limit 5 --dry-run --use-llm
```
...
実投入時に LLM 再順位付けを使わない場合は `--no-llm` を指定します。

```bash
bun src/services/knowflow/cli.ts seed-frontier --limit 5 --no-llm
```
...
常駐実行は既定で有効です。停止したい場合だけ `GNOSIS_ENABLE_AUTOMATION=false` または `GNOSIS_BACKGROUND_WORKER_ENABLED=false` を指定します。macOS LaunchAgent で運用する場合は `scripts/setup-automation.sh install` で各 plist を配置し、`scripts/setup-automation.sh status` または `bun run doctor` で読み込み状態を確認できます。
