...
Gnosis 本体の primary tool surface は以下の8件です。
...
- [memory_search](#memory_search): `vibe_memories` raw memory の薄い一覧取得
- [memory_fetch](#memory_fetch): raw memory の必要範囲だけを部分取得
...
- `doctor` の `toolVisibility.exposedToolCount` は Gnosis primary tools として `8` です。
...
- **処理**:
  ...
  - `memory_search` / `memory_fetch` も内部ツールとして使えますが、第一ラウンド prefetch には含めません。context 圧縮で raw memory の詳細確認が必要な場合だけ補助的に呼びます。
...
- **用途**: `vibe_memories` に保存された raw memory を、Web 検索の search と同じ薄い一覧として確認します。context 圧縮後に過去会話・作業断片・`agentic_search` 保存回答の詳細が必要な場合の補助導線です。
- **位置づけ**: entity knowledge の代替ではありません。通常は `agentic_search` / `search_knowledge` / `review_task` を優先し、圧縮 summary だけでは足りない場合に使います。
- **検索方式**:
  - `mode: "vector"` は `generateEmbedding(query, { type: "query", priority: "high" })` で query embedding を作り、`vibe_memories.embedding` と照合します。
  - `mode: "like"` は embedding を作らず、本文の phrase match を検索します。
  - `mode: "hybrid"` は vector と LIKE を併用し、同じ memory id を merge します。embedding が使えない場合も LIKE 結果を返し、`degraded.code = "EMBEDDING_UNAVAILABLE"` を付けます。
- **入力**:
  - `query` (string, 必須)
  - `mode` (`hybrid|vector|like`, 任意、既定 `hybrid`)
  - `limit` (number, 任意、既定 `5`, 最大 `20`)
  - `sessionId` (string, 任意)
  - `memoryType` (`raw`, 任意、MVP は `raw` のみ)
  - `maxSnippetChars` (number, 任意、既定 `240`, 最大 `1000`)
- **出力**:
  - `items[]`: `id`, `sessionId`, `createdAt`, `source`, `matchSources`, `score`, `snippet`
  - `retrieval`: `query`, `mode`, hit counts, `embeddingStatus`
  - `degraded`（必要時）
- **非出力**: `vibe_memories.metadata` や raw JSON 全体は返しません。

### `memory_fetch`

- **用途**: `memory_search` で得た memory id から、必要な範囲だけを抽出します。Web 検索の fetch に相当します。
- **入力**:
  - `id` (string, 必須)
  - `query` (string, 任意)
  - `start` (number, 任意、0-based UTF-16 index、inclusive)
  - `end` (number, 任意、0-based UTF-16 index、exclusive)
  - `maxChars` (number, 任意、既定 `1000`, 最大 `5000`)
- **範囲決定**:
  - `start` または `end` が指定された場合は explicit range を優先し、`query` match は使いません。
  - `start` のみなら `start` から `maxChars`、`end` のみなら `end - maxChars` から `end` を返します。
  - explicit range が `maxChars` を超える場合は `maxChars` で上限をかけ、`truncated: true` を返します。
  - explicit range がない場合、`query` の phrase match、なければ2文字以上 token match の周辺を既定1000文字で返します。
  - match がない、または `query` がない場合は先頭1000文字を返し、`degraded.code = "NO_EXACT_EXCERPT_MATCH"` を付けます。
- **出力**:
  - `range`: `start`, `end`, `totalChars`, `source`
  - `excerpts[]`: `text`, `matched`, `start`, `end`
  - `text`, `truncated`, `degraded`（必要時）
- **参照記録**: 対象 memory が見つかった場合は `referenceCount` / `lastReferencedAt` を更新します。
